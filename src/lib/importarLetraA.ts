...
/** Lee una hoja con encabezados en la fila 4 y datos desde la fila 5, igual que el Excel de Casinos/Bancos. */
function leerHoja(sheet: XLSX.WorkSheet): Fila[] {
  const raw = XLSX.utils.sheet_to_json<unknown[]>(sheet, { header: 1, defval: null, range: 3 });
  ...
  return raw
    ...
    .map((row) => {
      const fila: Fila = {};
      headers.forEach((h, i) => {
        if (h) fila[String(h)] = row[i] ?? null;
      });
      return fila;
    })
    .filter((fila) => Object.values(fila).some((v) => v !== null && v !== undefined && v !== ""));
}

/** Extrae las hojas "Casinos A" y "bancos Gral" de un Excel de cuentas letra A. */
export function parseWorkbookLetraA(buffer: Buffer): { casinos: Fila[]; bancos: Fila[] } {
  const wb = XLSX.read(buffer, { type: "buffer", cellDates: true });
  const hojaCasinos = wb.Sheets["Casinos A"];
  const hojaBancos = wb.Sheets["bancos Gral"];
  if (!hojaCasinos || !hojaBancos) {
    throw new Error('El Excel debe tener las hojas "Casinos A" y "bancos Gral".');
  }
  return { casinos: leerHoja(hojaCasinos), bancos: leerHoja(hojaBancos) };
}
...
async function importarCasinos(filas: Fila[]) {
  ...
  for (const row of filas) {
    ...
    const notaPartes = [textoLimpio(row["Nota, seguimiento"]), textoLimpio(row["Accion"])].filter(
      (x): x is string => !!x,
    );
...
async function importarBancos(filas: Fila[]) {
  ...
  return { creados, actualizados, omitidos, clientesVinculados };
}

/** Importa a Casino y Cuenta el Excel de cuentas letra A (hojas "Casinos A" y "bancos Gral"). Solo ADMIN. */
export async function importarLetraA(actor: Actor, buffer: Buffer) {
  assertAdmin(actor);
  const { casinos, bancos } = parseWorkbookLetraA(buffer);
  const resultadoCasinos = await importarCasinos(casinos);
  const resultadoBancos = await importarBancos(bancos);
  return { casinos: resultadoCasinos, bancos: resultadoBancos };
}
